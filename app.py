...
def room_preview():
    ...
    query  = "SELECT * FROM Rooms WHERE available_beds > 0"
    params = []
...
def delete_booking(booking_id):
    ...
    """, (booking_id,)).fetchone()['cnt']

    if unpaid > 0:
        conn.close()
        flash('Cannot delete this booking — it has unpaid payment records. Resolve payment first.', 'danger')
        return redirect(url_for('manage_booking'))
...
def process_payment():
    ...
    add_notification(
        conn,
        booking['student_id'],
        f"Payment of RM {booking['total_payment']:.2f} for Room {booking_id} was successful.",
        'success'
    )
...
def download_payment_pdf():
    ...
    payments = conn.execute("""
        SELECT p.payment_id, p.payment_method, p.payment_date, p.amount, 
               p.payment_status, b.student_name, r.room_number
        FROM Payment p
    ...
    # Table data
    table_data = [['ID', 'Student', 'Room', 'Amount', 'Method', 'Date', 'Status']]
    for p in payments:
        table_data.append([
            str(p['payment_id']),
            p['student_name'] or 'N/A',
            p['room_number'] or 'N/A',
            f"RM {p['amount']:.2f}",
            p['payment_method'],
            p['payment_date'],
            p['payment_status']
    ...
    table = Table(table_data,
                  colWidths=[0.5 * inch, 1.5 * inch, 0.8 * inch, 1 * inch, 1.2 * inch, 1 * inch, 0.8 * inch])
    table.setStyle(TableStyle([
    ...
    if student:
        ...
        if room_number != 'N/A':
           my_requests = conn.execute("""
            SELECT request_id as id, request_id,description, request_date as report_date, maintenance_status
...
def submit_maintenance():
    ...
    conn.execute("""
        ...
        (student_id, room_id, description, category, maintenance_status, request_date, priority)
        VALUES (?, ?, ?, ?, 'pending', ?,?)
    """, (student_id, room_id, description, category,
          datetime.now().strftime('%Y-%m-%d'), priority))
...
def student_edit_profile():
    ...
    try:
        ...
        cursor.execute("""
            UPDATE Students SET programme = ?, year_of_study = ?, emergency_contact = ?, gender = ?
            WHERE user_id = ?
        """, (programme, year_of_study, emergency_contact, gender, user_id))
...
- Fines: Wardens may issue fines (e.g. RM 50) for policy violations or overdue payments.
  Fines appear on the student's Payment History page.
- Maintenance requests: Students submit issues via "Maintenance Request" by selecting
  a category and writing a description. Priority (High/Medium/Low) is assigned
  automatically based on category. Wardens mark requests as done once resolved.
